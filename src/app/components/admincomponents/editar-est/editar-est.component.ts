...
export class EditarEstComponent implements OnInit {
  ...
  // getUserId() {
  //   this.usr.getUserOnlyId(this.id_user).subscribe(
  //     (user: any) => {
  //       this.userForm.patchValue(user);
  //       this.user_data = user;
  //       console.log(this.user_data); // Muestra los datos en la consola después de obtenerlos
  //     },
  //     (error) => {
    ...
    if (this.userForm && this.userForm.valid) {
      this.usr.updateUserById(this.id_user, this.userForm.value).subscribe(
        () => {
          ...
          this.goodNot();
          this.router.navigate(['homeadmin/estudiantes']);
